...
def run():
    ...
    info()
    step = 0
    ss =25; cm=0;  #paprameters
    #timeofcarcross = 0.2; # time requires for one car to cross signal
    Coeff = measure()
    while step < 3600:
        ...
        if step%ss == 0: 
          ...
          #print(Coeff[(opt_signal)])
          g_opt = qubo(measure());
          g_phase = qubo2phase(g_opt);
          print(g_phase)
          print(current_phase)
          if abs (g_phase - opt_phase) !=0:
              time = 20;              
          else:
              time = 40;
          # if the qubo tell signle should be changed
          if abs(opt_phase - current_phase) != 0:
              traci.trafficlight.setPhase("A0",opt_phase)    ##setting phase
